  ...
  <Popover v-model:open="open">
    ...
    <PopoverContent class="w-80 p-0 bg-card" align="end">
      ...
      <div class="flex items-center justify-between p-4 border-b border-border">
        ...
        <!-- <Button v-if="unreadCount > 0" variant="ghost" size="sm" class="text-xs h-7" @click="markAllAsRead">
          Mark all read
        </Button> -->
      </div>
        ...
        <!-- <div v-if="notifications.length > 0" class="divide-y divide-border">
          <div v-for="notification in notifications" :key="notification.id"
            ...
            <div class="flex-1 min-w-0">
              <RouterLink v-if="notification.link" :to="notification.link" @click.native="
...
<div class="opacity-0 group-hover:opacity-100 transition-opacity flex items-start gap-1">
  <Button v-if="!notification.read" variant="ghost" size="icon" class="h-6 w-6" @click="markAsRead(notification.id)">
    <Check class="w-3 h-3" />
  </Button>
  <Button variant="ghost" size="icon" class="h-6 w-6 text-destructive hover:text-destructive"
    @click="deleteNotification(notification.id)">
    <X class="w-3 h-3" />
  </Button>
</div>

<div v-if="!notification.read" class="absolute left-1.5 top-1/2 -translate-y-1/2 w-2 h-2 bg-primary rounded-full" />
</div>
</div>
...
<div v-else class="flex flex-col items-center justify-center py-12 text-muted-foreground">
  ...
  <p class="text-sm">No notifications</p>
      ...
      <!-- <div v-if="notifications.length > 0" class="p-3 border-t border-border">
        <Button variant="ghost" size="sm" class="w-full text-destructive hover:text-destructive" @click="clearAll">
